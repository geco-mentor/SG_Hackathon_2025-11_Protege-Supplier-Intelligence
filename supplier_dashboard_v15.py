"""
Wrapper to auto-install key third-party dependencies for this dashboard.
This block is added automatically and does NOT change any of the business logic below.
"""

import sys
import subprocess

def ensure_installed(package_name: str):
    """
    Ensure a pip package is installed.

    Tries to import the top-level module. If ImportError, runs:
        python -m pip install <package_name>
    using the same interpreter that is running this script.
    """
    try:
        ...
        try:
            ...
            # If installation fails, we just propagate a clear error later when importing.
            print(f"[AUTO-INSTALL] Failed to install {{package_name}}: {{e}}")

# Core external dependencies used in this dashboard.
# Add to this list if you later introduce more third-party packages.
for pkg in [
    "streamlit",
    "pandas",
    "altair",
    ...
    if df_tab.empty:
        ...
        with col_ai:
            with st.expander(" **Insights & Recommendations**", expanded=True):
                ...
                # Generate button
                if st.button("Generate Insights & Recommendations", type="primary", width='stretch', key="tab1_gen_ai"):
            ...
            with col_tariff:
                ...
                if not df_tariff.empty:
                    ...
                    if not df_tariff_filtered.empty:
                        ...
                        if tariff_chart:
                            st.altair_chart(tariff_chart, width='stretch')
        ...
        if not df_sup_f.empty:
            ...
            if df_sup_overview.empty:
                ...
                if existing_cert_cols:
                    ...
                    st.dataframe(supplier_table, width='stretch', height=400, hide_index=True)
    ...
    with st.expander(" **Supplier Document Status**", expanded=False):
        ...
        if not docs_table.empty:
            ...
            if existing_cert_cols:
                ...
                st.dataframe(docs_table, width='stretch', height=400, hide_index=True)
