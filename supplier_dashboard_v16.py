"""
Cross-platform bootstrap for this dashboard.

- Auto-installs required third-party pip packages if they are missing.
- Ensures the project root (this file's folder) is on sys.path so `import api.*` works
...
def ensure_installed(package_name: str, module_name: str = None):
    """
    Ensure a pip package is installed.

    package_name: name used in `pip install`
    module_name:  name used in `import` (defaults to package_name)
    """
    ...
    try:
        ...
        try:
            ...
            # If installation fails, we do not crash here; the normal import will fail later
            # with the usual ModuleNotFoundError, which is clearer for debugging.
            print(f"[AUTO-INSTALL] Failed to install {package_name}: {e}")

# Core third-party dependencies used in this dashboard
for pkg, mod in [
    ("streamlit", "streamlit"),
    ("pandas", "pandas"),
    ("altair", "altair"),
    ...
    ensure_installed(pkg, mod)

# Ensure the directory containing this file is on sys.path so that `import api.*` works.
